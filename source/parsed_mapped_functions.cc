...
ParsedMappedFunctions<spacedim>::ParsedMappedFunctions(
...
ParsedMappedFunctions<spacedim>::add_normal_components()
{
  ...
  for (unsigned int i = 0; i < var.size(); ++i)
    {
      int n =
        std::count(_component_names.begin(), _component_names.end(), var[i]);
      if (n > 1)
        {
          _normal_components.push_back(var[i] + ".N");
          int pos = std::find(_component_names.begin(),
                              _component_names.end(),
                              var[i]) -
                    _component_names.begin();
  ...
  for (it_type it = id_components.begin(); it != id_components.end(); ++it)
    {
      std::pair<ComponentMask,
        ...
        mapped(it->second, id_functions[it->first]);
  ...
  for (unsigned int i = 0; i < idcomponents.size(); ++i)
    {
      ...
      AssertThrow(components.size() <= n_components,
                  ExcMessage("Wrong number of components specified for id " +
                             id_comp[0]));
  ...
  const std::string &constants)
{
  ...
  // check if the number of ids defined in id_components and id_functions are
  // the same
  AssertThrow(
    ids.size() == id_defined_functions.size(),
    ExcMessage(
...
template <int spacedim>
shared_ptr<dealii::Functions::ParsedFunction<spacedim>>
ParsedMappedFunctions<spacedim>::get_mapped_normal_function(
...
template <int spacedim>
shared_ptr<dealii::Functions::ParsedFunction<spacedim>>
ParsedMappedFunctions<spacedim>::get_mapped_function(
...
template <int spacedim>
ComponentMask
ParsedMappedFunctions<spacedim>::get_mapped_mask(const unsigned int &id) const
{
  Assert(mapped_functions.find(id) != mapped_functions.end(),
         ExcIdNotFound(id));
...
ParsedMappedFunctions<spacedim>::get_mapped_ids() const
{
  return ids;
}
...
template <int spacedim>
std::vector<unsigned int>
ParsedMappedFunctions<spacedim>::get_mapped_normal_ids() const
{
  return normal_ids;
}
...
ParsedMappedFunctions<spacedim>::set_time(const double &t)
{
  typedef typename std::map<
    unsigned int,
    shared_ptr<dealii::Functions::ParsedFunction<spacedim>>>::iterator it_type;
...
ParsedMappedFunctions<spacedim>::set_normal_functions()
{
  ...
  for (it_type it = mapped_normal_components.begin();
       it != mapped_normal_components.end();
       ++it)
    {
      ...
      for (unsigned int i = 0; i < normal_ids.size(); ++i)
        {
          ...
          normal_func =
            Utilities::split_string_list(id_str_functions[normal_ids[i]], ';');
