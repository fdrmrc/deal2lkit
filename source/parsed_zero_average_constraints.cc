...
ParsedZeroAverageConstraints<dim, spacedim>::ParsedZeroAverageConstraints(
  ...
  for (unsigned int c = 0; c < boundary_components.size(); ++c)
    {
      if ((std::find(_component_names.begin(),
                     _component_names.end(),
                     boundary_components[c]) != _component_names.end()))
        for (unsigned int j = 0; j < _component_names.size(); ++j)
          boundary_mask[j] =
            (_component_names[j] == boundary_components[c] || mask[j]);
...
template <int dim, int spacedim>
ComponentMask
ParsedZeroAverageConstraints<dim, spacedim>::get_boundary_mask() const
{
